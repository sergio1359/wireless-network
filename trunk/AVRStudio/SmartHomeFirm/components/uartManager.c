...
static UARTReceiverState_t uartState = USART_RECEIVER_IDLE_RX_STATE;
...
void USART_SendOperation(INPUT_UART_HEADER_t* input_header, OPERATION_HEADER_t* operation_header, void* callback)
{
	...
	if( callback != 0)
	{
		uartDataConf.header = operation_header;
		((void (*)(OPERATION_DataConf_t *))callback)(&uartDataConf);
	}
}
