...
namespace DomoticNetwork.Plugin
{
    ...
    class PluginLoader
    {
        ...
        public List<IPlugin> Modules { get; private set; }
                ...
                try
                {
                    ...
                    foreach (Type moduleType in moduleAssembly.GetTypes())
                    {
                        ...
                        if (moduleType.IsPublic && !moduleType.IsAbstract && moduleType.IsClass)
                        {
                            ...
                            if (moduleType.GetInterface(typeof(IPlugin).FullName, true) != null)
                            {
                                IPlugin i = (IPlugin)Activator.CreateInstance(moduleAssembly.GetType(moduleType.ToString()));
                                Modules.Add(i);
                                //break;
                            }
                        }
